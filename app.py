...
def update():
    ...
    if name in ['Can', 'Status', 'Comment', 't0', 'tE', 'Kmag', 'r_K']:
        ...
        if name == 'Comment':
            value = f'\'{value}\''
        db.engine.execute("UPDATE Src SET {} = {} WHERE id = {} ".format(name, value, pk))
        db.session.commit()
...
def src(Event, chartID='chart_ID'):
    ...
    for i in config.tags:
        if i == 'Ongoing':
            ...
            tg[i] = 1*(x[i] > 0)
    return render_template('src.html', pageType=pageType, chartID=chartID, chart=chart,
                           series=series, title=title, src=x, pars=pars, models=models ,fit_info='success',tags=tg, prev=prev, next=next)
...
def fitparm():
    ...
    t0 = 9750. if t0=='None' else float(t0)
    tE = 50. if tE=='None' else float(tE)
...
def updatetags():
    ...
    if not checked:
        x = x.replace(tag, '')
        db.engine.execute("UPDATE Src SET {} = \'{}\' WHERE id = {} ".format('Tags', x, pk))
        if tag in ['AT;','UT;','AT_wide;','UT_wide;']:
            db.engine.execute("UPDATE Src SET {} = 0 WHERE id = {} ".format(tag[:-1],pk))
        if tag == 'Ongoing;':
        ...
        if tag in ['AT;','UT;','AT_wide;','UT_wide;']:
            db.engine.execute("UPDATE Src SET {} = 1 WHERE id = {} ".format(tag[:-1],pk))
