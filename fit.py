...
def fit(event_name, u_0, t_0, t_E, bl=False):
    ...
    t = np.concatenate([np.linspace(t0-2*tE,t0-0.2*tE,100),np.linspace(t0-0.2*tE,t0+0.2*tE,100),np.linspace(t0+0.2*tE,t0+2*tE,100)])
    model_mag = flux2mag([PSPL(t,t0,u_0,tE)*f[0]+f[1]])
    np.savetxt(model_file,np.array([t,model_mag]).T,fmt="%12.4f %8.3f")
    np.savetxt(model_par,np.array([res.fun,t0,u_0,tE,flux2mag([f[0]]),flux2mag([f[1]+f[0]])]))
    return 'success'
...
def loadfit(event_name):
    ...
    for par in par_files:
        ...
        fit = {'u0': para[2], 't0': para[1], 'tE': para[3], 'Is': para[4], 'Ib': para[5], 'bl': bl,
               'chi2': para[0],'name':f'{u0bl}'}
        df = np.loadtxt(model)
        if df[0][0] < 2450000:
            df = df.T
            df[0] = df[0] + 2450000
