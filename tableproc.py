...
dtypes = ['S32','S32','S16','S32','S32','f4','f4','f2','f4','f2','f2','f2','f2','f2','f2','f2','f2','f2','f2','f2','f2','f2','i4','i4','i4','i4','i4','i4','S128','S128']
dvalues = ['--','--','--','--','--',0,0,0,0,0,0,0,0,99.99,0,0,0,0,0,99.99,0,99.99,-1,-1,-1,-1,-1,-1,'--',';']
formatter = {'t0':'.2f','u0':'.3f','tE':'.2f','Kest':'.2f','Kmag':'.2f','Gmag':'.2f','r_K':'.2f','gbase':'.2f','rbase':'.2f','ibase':'.2f','G_FT':'.2f','K_FT':'.2f','r_FT':'.2f','G_AO':'.2f','r_AO':'.2f'}
...
def query_table(ra_dec,catalog='gaiadr3',radius='30s',column_filters={}):
    ...
    v.ROW_LIMIT=-1
    res = v.query_region(coord.SkyCoord(ra_dec,unit=(u.hourangle, u.deg),frame='icrs'),radius=radius,catalog=v_catalog,column_filters=column_filters)
    try:
        return res[0]
    except:
...
def cross_match(input_tab,input_cat,match_cat='gaiadr3',max_distance=1):
    ...
    res = XMatch.query(cat1=input_tab,cat2=f'vizier:{v_catalog}',max_distance=max_distance * u.arcsec, colRA1=keys[0],colDec1=keys[1])
    return res

def get_VLTI_refstars(ra_dec):
    G_FT = query_table(ra_dec,catalog='gaiadr3',radius='30s',column_filters={'Gmag':'<15'})
    G_AO = query_table(ra_dec,catalog='gaiadr3',radius='1m',column_filters={'Gmag':'<11'})
    if G_AO : G_AO.sort('Gmag')
    if not G_FT :
    ...
    FT_AT = FTs[np.logical_and.reduce((FTs['Kmag'] < 9 , FTs['Gmag'] < 13, FTs['_r'] > 2))]
    FT_UT = FTs[np.logical_and.reduce((FTs['Kmag'] < 11 , FTs['Gmag'] < 15, FTs['_r'] > 2))]
    FT_AT.sort('Gmag')
    FT_UT.sort('Gmag')
    return FT_AT,FT_UT,G_AO

# headers Event,PubDate,RA,Dec,l,b,t0,u0,tE,K_est,Gbase,Kbase,gbase,rbase,ibase,G_FT,K_FT,r_FT,AT,UT,AT_wide,UT_wide,Can,Comment,Tags
def vert(ra,dec):
    def d2t(deg):
        hh = deg // 15
        mm = (deg - hh*15)//0.25
...
# print(get_VLTI_refstars('12:14:23.54	-63:40:17.90'))
